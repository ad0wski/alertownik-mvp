...
## Stage 16 — Automatic Source Monitoring Engine 🟢 (Sprint 166C, branch only, ~80–85%)
...
- ✅ **Concurrency lock — corrected mid-sprint, then executed live and proven under a real two-concurrent-invocations test**: the first live wiring used a `SELECT` → decide → `INSERT` sequence, a genuine race between two concurrent invocations. Redesigned as a Postgres-atomic mechanism — a partial `UNIQUE INDEX` (at most one open run per `environment_tag`+`trigger`) plus two `SECURITY DEFINER` functions that are the *only* way to write the table. Migration (`docs/sql/PROPOSED_SPRINT_166C_ATOMIC_LOCK_MIGRATION_V2.sql`, Revision 2, hardened per a read-only security audit) executed exactly once against `alertownik-preview`; 9/9 read-only post-migration checks PASS.
- ✅ **Live two-concurrent-invocations test against the deployed Preview endpoint — PASS.** Exactly one of two simultaneous `write-candidates` requests processed (`ok:true`, 1 candidate, 1 source_check); the other was cleanly blocked (`ok:false`, `reason:"lock_held"`) before any fetch or write. Read-only verification: `alerts` unchanged (7), `source_notice_candidates` and `source_checks` each +1 (5), exactly one `scheduled_writer_runs` row (the blocked request created zero rows), that row correctly closed with `outcome=success` and counters matching the winning response exactly, zero rows with `finished_at IS NULL`. Production never touched. `SCHEDULED_WRITES_ENABLED` was returned to `false` for Preview immediately after; the test candidate and history row are kept as live evidence, not rolled back.
- ✅ Full test coverage for the atomic design against an in-memory model of the real unique-index semantics (exactly one winner of two simultaneous open attempts, stale-lock auto-recovery, fail-closed on any lock-mechanism error) — see `tests/e2e/scheduledWriterRouteHistoryLock.spec.ts` — plus 25 pure-function tests mirroring the migration's own parameter validation at its exact boundary values — see `tests/e2e/scheduledWriterRunSafety.spec.ts`.
- ❌ Not yet done, deliberately out of this sprint's scope: no cron entry (Stage 6); no alerting beyond the history row itself (Stage 4); no multi-source expansion beyond Michałowice (Stage 7).
- ❌ `SCHEDULED_CHECKS_ENABLED` left `true` for Preview throughout (unchanged); Production untouched throughout the entire sprint.

**Goal achieved:** genuine, database-enforced protection against two overlapping `write-candidates` runs, designed, migrated, and proven live against a real Preview deployment — before ever considering a real schedule (Stage 6). See `docs/SPRINT_166C_AUTOMATIC_SOURCE_MONITORING_AUDIT_AND_DESIGN_V1.md` for the full design, live-test results, and remaining (separately-gated) follow-up stages.
