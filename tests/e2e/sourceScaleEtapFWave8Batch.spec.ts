...
// voivodeships. 73 candidates personally HTTP-verified across the four
// voivodeships and only 2 cleared the GO bar (Dolnośląskie and Lubuskie
// honestly found zero GO despite broad, good-faith discovery). Same
// wordpress_rest mechanics as every prior wave — no new parser code. This
// file proves: batch shape, correct check-only activation, and — the part
// that must never become false — no writer/auto-publish reach, no write
// path. No network calls in this file.

const FALA8_IDS = ["mzd-opole", "ostroda-komunikaty"];

test.describe("Fala 8 — batch shape", () => {
  test("both instances registered in OFFICIAL_SOURCE_CHECKS with a valid wordpress_rest config", () => {
    for (const id of FALA8_IDS) {
  ...
  test("categories are roads and municipal, not water", () => {
    const categories = FALA8_IDS.map(
      (id) => OFFICIAL_SOURCE_CHECKS.find((s) => s.id === id)!.category
    );
    expect(categories).toContain("roads");
    expect(categories).toContain("municipal");
...
// (Opole road-works / Ostróda mixed roads-water-safety notices) — not a
...
function post(overrides: Partial<WordpressRestPost>): WordpressRestPost {
  return {
    ...
    link: "https://example-mzd-opole.pl/2026/07/30/test/",
    slug: "test",
    ...overrides,
  };
}

test.describe("parseTransportRoadsRestPosts handles Fala 8's roads/mixed-municipal shape", () => {
  test("a real-shaped road-closure fixture is proposed correctly", () => {
    const posts = [
      post({
        title: { rendered: "Zamknięcie fragmentu ulicy Głównej" },
        excerpt: {
          rendered:
            "<p>Od 1 sierpnia 2026 roku fragment ulicy Głównej zostanie czasowo zamknięty " +
            "w związku z pracami remontowymi. Prosimy o zachowanie ostrożności.</p>",
        },
      }),
    ];
    ...
    expect(proposals[0].title).toContain("Zamknięcie fragmentu ulicy");
