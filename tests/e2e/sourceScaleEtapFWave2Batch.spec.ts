...
// Etap F, Fala 2 (2026-07-30) — 4 HTTP-verified Mazowieckie water
// utilities, personally verified by the main agent (no subagent — see
// CLAUDE.md's post-incident subagent ban). Same wordpress_rest mechanics
// as every prior wave, zero new parser code. This file proves: batch
...
const FALA2_IDS = ["pwik-minsk-mazowiecki", "pwik-wyszkow", "pwik-pultusk", "zwik-nowy-dwor-mazowiecki"];

test.describe("Fala 2 — batch shape", () => {
  test("all 4 instances registered in OFFICIAL_SOURCE_CHECKS with a valid wordpress_rest config", () => {
    for (const id of FALA2_IDS) {
...
// Fictional fixture modeled on this wave's verified real shape (Mińsk
// Mazowiecki/Pułtusk water-outage notices) — not a reproduction of any
// real site's content, per this repo's existing test convention.
...
test.describe("parseWordpressRestPosts already handles Fala 2's shape unmodified", () => {
  test("a real-shaped water-outage fixture is proposed correctly", () => {
    const posts = [
      post({
        ...
        excerpt: {
          rendered:
            "<p>W związku z awarią sieci wodociągowej w dniu 30 lipca 2026 roku mogą wystąpić " +
            "przerwy w dostawie wody oraz spadki ciśnienia w wybranych rejonach miasta. " +
    ...
    expect(proposals[0].title).toContain("Awaria sieci wodociągowej");
