...
// Etap F, Fala 3 (2026-07-30) — first Łódzkie voivodeship sources, 6
// HTTP-verified water utilities (each personally double-checked by the
// main agent, no subagent — see CLAUDE.md's post-incident subagent ban).
// Same wordpress_rest mechanics as every prior wave. This file proves:
// batch shape, correct check-only activation, and — the part that must
// never become false — no writer/auto-publish reach, no write path. No
...
// (Piotrków/Aleksandrów Łódzki/Wieluń water-outage notices) — not a
...
test.describe("parseWordpressRestPosts already handles Fala 3's shape unmodified", () => {
  test("a real-shaped 'brak wody' outage fixture is proposed correctly", () => {
    const posts = [
      post({
        title: { rendered: "Brak wody !" },
        excerpt: {
          rendered:
            "<p>W związku z awarią na sieci wodociągowej, w dniu 30 lipca 2026 roku w godzinach " +
            "10:00-13:00 wstrzymana zostanie dostawa wody w wybranych rejonach miasta. " +
    ...
    expect(proposals[0].title).toContain("Brak wody");
