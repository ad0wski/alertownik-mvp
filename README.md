...
>
> **Isolated Preview environment — complete and released to Production (Sprint 165C):** a real, separate Supabase project (`alertownik-preview`, West Europe/London, Free plan) exists with its own schema/RLS (replayed and verified read-only: 8 tables, 28 RLS policies, 4 triggers, exact match with Production's live snapshot), its own test admin and scheduled-writer Auth accounts, and synthetic-only seed data. Vercel's Preview environment points at this project (`NEXT_PUBLIC_SUPABASE_URL`, publishable key, `SUPABASE_ENVIRONMENT_TAG`, `SUPABASE_EXPECTED_PROJECT_REF` all Preview-scoped, separate from Production's own unchanged values, each using its own distinct project ref) — verified live: every Supabase call from a Preview deployment targets `alertownik-preview`, the `EnvironmentBadge` reads `PREVIEW`, and all visible content is synthetic. A genuine, previously-unknown bug (`alert_sources.url` nullable-URL crash on `/admin/sources`) was found via the synthetic seed's own null-URL test case and fixed (Sprint 165C-1), confirmed live on Production. Final test suite before release: 682/682 e2e, 17/17 PWA, `npm run check` and `build` all clean. The feature branch was fast-forward-merged to `main` and deployed to Production with zero regressions found on a full read-only smoke test of both environments. Automation (`SCHEDULED_CHECKS_ENABLED`/`SCHEDULED_WRITES_ENABLED`) remains fully OFF everywhere — activating it is a separate, later decision. See `docs/SPRINT_165C_FINAL_ACCEPTANCE_V1.md` and `docs/SPRINT_165C_POST_RELEASE_CLOSURE_V1.md`.
>
> **First controlled canary write, then back to OFF (Sprints 166A–166B, branch only — not merged to `main`):** on the isolated Preview environment, `CRON_SECRET` and `SUPABASE_SCHEDULED_WRITER_PASSWORD` were configured (Adam-generated/pasted directly into Vercel, never seen by Claude), `SCHEDULED_CHECKS_ENABLED` and `SCHEDULED_WRITES_ENABLED` were each flipped to `true` (Preview scope only) in separate, explicitly-approved steps, and exactly one controlled `write-candidates` call was made. Result: 1 `pending` candidate and 1 `source_checks` row inserted for the Michałowice source, hard cap correctly enforced (6 proposals found, 5 capped), zero alerts created or published — verified read-only against `alertownik-preview` only. `SCHEDULED_WRITES_ENABLED` was then returned to `false` and a follow-up deployment confirmed `write-candidates` is fail-closed again. Production was never read or modified at any point. The one `pending` candidate remains in place, awaiting ordinary manual admin review. See `docs/SPRINT_166A_PREVIEW_CANARY_REHEARSAL_AUDIT_AND_DESIGN_V1.md` and `docs/SPRINT_166B_DISABLED_CANARY_INFRASTRUCTURE_V1.md`.
