...
>
> **Safe auto-candidate canary foundation (Sprint 164B, branch only — not merged/deployed):** a new read-only "Stan automatyzacji (canary)" panel on `/admin/sources` shows, for the first time, whether the existing Michałowice candidate-automation pipeline's kill switches are on or off — never their secret values. The pipeline itself (Sprints 147–153) already satisfied every safety requirement this sprint set out to add (max 1 candidate per run, single-source allowlist, pending-only inserts, fail-closed on any missing switch/credential) — confirmed by a fresh audit, not reimplemented. Still OFF in every environment; activation is a separate, manual, staged process — see `docs/SPRINT_164B_SAFE_AUTO_CANDIDATE_CANARY_FOUNDATION_V1.md` and its paired activation/rollback runbooks.
