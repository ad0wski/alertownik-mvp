...
>
> **Isolated Preview environment — complete (Sprint 165C, branch-level acceptance; not yet merged to `main`):** a real, separate Supabase project (`alertownik-preview`, West Europe/London, Free plan) exists with its own schema/RLS (replayed and verified read-only: 8 tables, 28 RLS policies, 4 triggers, exact match with Production's live snapshot), its own test admin and scheduled-writer Auth accounts, and synthetic-only seed data. Vercel's Preview environment now points at this project (`NEXT_PUBLIC_SUPABASE_URL`, publishable key, `SUPABASE_ENVIRONMENT_TAG`, `SUPABASE_EXPECTED_PROJECT_REF` all Preview-scoped, separate from Production's unchanged values) — verified live: every Supabase call from a Preview deployment targets `alertownik-preview`, the `EnvironmentBadge` reads `PREVIEW`, and all visible content is synthetic. A genuine, previously-unknown bug (`alert_sources.url` nullable-URL crash on `/admin/sources`) was found via the synthetic seed's own null-URL test case and fixed (Sprint 165C-1). Automation (`SCHEDULED_CHECKS_ENABLED`/`SCHEDULED_WRITES_ENABLED`) remains fully OFF everywhere — activating it is a separate, later decision. See `docs/SPRINT_165C_FINAL_ACCEPTANCE_V1.md`.
