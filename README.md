...
>
> **Isolated Preview infrastructure — project created, schema/RLS replayed (Sprint 165C, in progress):** a real, separate Supabase project (`alertownik-preview`, West Europe/London, Free plan) now exists, and its schema/RLS have been replayed and verified read-only — 8 tables, 28 RLS policies, 4 triggers, all RLS-enabled, exact match with Production's live snapshot, every table at 0 rows. No Supabase Auth account, no seed data, and no Vercel environment variable change has happened yet — Vercel Preview still points at Production's original Supabase project. See `docs/SPRINT_165C_MANUAL_GATE_2_PROJECT_CREATED_V1.md` and `docs/SPRINT_165C_PHASE_3_SCHEMA_RLS_REPLAY_V1.md`.
