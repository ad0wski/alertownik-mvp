...
>
> **Isolated Preview code safety package (Sprint 165B, code + docs — no infrastructure yet):** the code half of Sprint 165A's design is now live in the codebase — a single-source-of-truth environment identity (`VERCEL_ENV`-derived), a visible `PRODUCTION`/`PREVIEW`/`DEVELOPMENT`/`UNKNOWN` badge in the admin panel, and a new fail-closed `SUPABASE_ENVIRONMENT_TAG` pairing guard added as a fourth, additive gate on the one write-capable automation route (`write-candidates`). No value is configured for the new variable anywhere, so every environment (including Production) is blocked by this gate today, exactly as it already was for other reasons — no infrastructure, SQL, or automation was touched. See `docs/SPRINT_165B_ISOLATED_PREVIEW_CODE_SAFETY_PACKAGE_V1.md`.
